...
export async function generateStaticParams() {
  return enSavoir.map(q => ({ slug: q.slug }));
}

export default async function EnSavoirPage(props: { params: Promise<{ slug: string }> }) {
  const params = await props.params;
  const item = enSavoir.find(q => q.slug === params.slug);
